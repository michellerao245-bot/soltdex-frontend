...
function App() {           
  ...
  const [userAccount, setUserAccount] = useState(null); // Wallet address store karne ke liye

  const toggleSidebar = () => setIsOpen(!isOpen);           
  const sidebarWidth = isOpen ? '260px' : '80px';      

  // --- AUTOMATIC REFERRAL TRACKING START ---
  useEffect(() => {
    const urlParams = new URLSearchParams(window.location.search);
    const ref = urlParams.get('ref');
    if (ref) {
      // Agar URL mein ?ref=0x... hai, toh use browser memory mein save kar lo
      localStorage.setItem('soltdex_referrer', ref);
      console.log("Referrer caught and saved:", ref);
    }

    // Connected wallet check karne ka logic (optional agar aapka ConnectWallet component ye handle karta hai)
    const savedAccount = localStorage.getItem('walletAddress');
    if (savedAccount) setUserAccount(savedAccount);
  }, []);
  // --- AUTOMATIC REFERRAL TRACKING END ---
        ...
        >           
          <header className="nav-container">             
            <div className="nav-right">        
              {/* ConnectWallet ko ek function pass karo taaki wo App.js ko address bata sake */}
          ...
          <div className="content-wrapper">  
            <div className="main-content">             
              <Routes>             
                ...
                {/* Dashboard ko userAddress pass kar rahe hain */}
